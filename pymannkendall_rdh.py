...
def multivariate_test(x_old, alpha = 0.05):
    ...
    # x_old = np.transpose(np.array(df))
...
#     x, n = __missing_values_analysis(x, method = 'skip')  # It makes all column at the same size
    ...
    #         break
    #
    # x_skinny = x[:,nan_row:-(nan_row + 1)].copy()
    ...
    for i in range(c):
        if c == 1:
            ...
            # x_new, n = __missing_values_analysis(x_skinny, method = 'skip')  # It makes all column at deferent size

        # x_new_expanded = np.zeros([np.shape(x_new)[0], np.shape(x_new)[1]+2*nan_row])
        # x_new_expanded[:] = np.nan
        # x_new_expanded[:, nan_row:-nan_row] = x_new
        # x_new = x_new_expanded.copy()

        # detrending
        slope, intercept = sens_slope(x_old[:,i])
        x_detrend = x_new - np.arange(1, n + 1) * slope
        # x_detrend = np.transpose(x_new) - np.arange(1, n + 1) * slope

        # PreWhitening
        acf_1 = __acf(x_detrend, nlags=1)[1]
        a = range(0, n - 1)
        b = range(1, n)
        x_new = x_detrend[b] - x_detrend[a] * acf_1

        n = len(x_new)
        x_new = x_new + np.arange(1, n + 1) * slope
